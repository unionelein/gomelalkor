    ...
    public function pageAction($typeName, $page)
    {
        ...
        switch ($type) {
            case AppManager::POKUPKA:
                $title = 'Магазин стройматериалов. Купить стройматериалы в Гомеле'.($page > 1 ? '. Страница '.$page : '').' | Алькор';
                $description = 'Купить строительные материалы по хорошим ценам. Большой выбор стройматериалов.  При заказе от ' . $deliveryPrice . 'р доставка - бесплатно!' . ($page > 1 ? ' Страница '.$page : '');
                break;
            case AppManager::PROKAT:
                $title = 'Прокат инструмента в Гомеле. Аренда электроинструмента'.($page > 1 ? '. Страница '.$page : '').' | Алькор';
                $description = 'Прокат электроинструмента в Гомеле. Широкий выбор: бензопилы, перфораторы, дрели и др. При заказе от ' . $deliveryPrice . 'р доставка - бесплатно!' . ($page > 1 ? ' Страница '.$page : '');
                break;
            default:
                $title = null;
                $description = null;
